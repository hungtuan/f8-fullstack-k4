...
function updateLyrics(currentTime) {
  ...
  for (var i = currentLyricIndex; i < sentences.length; i++) {
    ...
    for (var j = 0; j < words.length; j++) {
      ...
      if (currentTime >= word.startTime && currentTime < word.endTime) {
        ...
        var lyricText = word.data;
        var lyricsElement = document.querySelector(".lyrics");
        lyricsElement.innerText = lyricText;

        // Lưu lại chỉ số hiện tại
        currentLyricIndex = i;
  ...
  // Nếu không tìm thấy từ nào, xóa nội dung lời bài hát
  var lyricsElement = document.querySelector(".lyrics");
