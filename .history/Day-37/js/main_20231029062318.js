...
const app = {
  ...
  getProfile: async function () {
    try {
      ...
      let htmlPostNew = `<h1>Create a New Post</h1>
      <form id="post-form">
          ...
          <div class="form-group">
              <label for="post-time">Set Time to Post!</label>
              <input type="datetime-local" id="post-time" name="post-time">
              <p class="error" id="post-time-error">Can't use now</p>
          </div>
          <div class="form-group">
              <button type="submit" id="submit-button">Write New!</button>
          </div>
      </form>`;
      container.append(htmlPostNew.innerHTML);
      const postsElement = await this.renderPosts();
